...
#     tmp_salt = bcrypt.gensalt()
#     tmp_hashed = bcrypt.hashpw(password.encode(), tmp_salt)
#     salt = str(tmp_salt.decode())
#     hashed = str(tmp_hashed.decode())
#     return hashed, salt


# def hash_password(password):
#     """
#     Genera un hash della password e restituisce l'hash insieme al salt.
#     """
#     # Genera un salt
#     salt = bcrypt.gensalt()
#     # Genera l'hash della password
#     hashed_password = bcrypt.hashpw(password.encode('utf-8'), salt)
#     # Restituisce l'hash e il salt come stringhe codificate
#     return hashed_password.decode('utf-8'), salt.decode('utf-8')


def hash_password(password):
    """Genera un hash e un salt per la password."""
    salt = bcrypt.gensalt()
    hashed_password = bcrypt.hashpw(password.encode('utf-8'), salt)
    # Codifica il salt in base64 per salvarlo nel database
    encoded_salt = base64.b64encode(salt).decode('utf-8')
    return hashed_password.decode('utf-8'), encoded_salt


# def verify_password(password, stored_hash, stored_salt):
#     """
#     Verifica se la password fornita corrisponde all'hash e al salt salvati.
#     """
#     # Converti il salt e l'hash in formato byte
#     salt_bytes = stored_salt.encode('utf-8')
#     stored_hash_bytes = stored_hash.encode('utf-8')
#     # Genera l'hash della password fornita usando il salt salvato
#     new_hash = bcrypt.hashpw(password.encode('utf-8'), salt_bytes)
#     # Confronta i due hash
#     return new_hash == stored_hash_bytes

def verify_password(password, stored_hash, encoded_salt):
    """Verifica la password con l'hash e il salt."""
    try:
        # Decodifica il salt da base64
        salt_bytes = base64.b64decode(encoded_salt.encode('utf-8'))
        salt_bytes = sanitize_hash(salt_bytes)
        # Genera un nuovo hash con la password fornita e il salt decodificato
        new_hash = bcrypt.hashpw(password.encode('utf-8'), salt_bytes)
        # Confronta gli hash
        new_hash = sanitize_hash(new_hash)
    ...
    try:
        ...
        if response.status_code == 200:  # se l'email è presente nel db possiamo procedere con l'encrypt della
            ...
            if verify_password(password, stored_hash, salt):
                ...
                # print(tmp)
                return jsonify(
                    {"error": "Invalid password credentials", "password": password, "stored_hash": stored_hash,
                     "salt": salt}), 400
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            #     return jsonify({"message": "Login successful"}), 200
                ...
                return jsonify({"error": "Invalid password credentials", "stored_hash": stored_hash, "salt": salt}), 400
        elif response.status_code == 400:
            return jsonify({"error": "Invalid credentials"}), 400
        else:
            return jsonify({"error": "Internal server error"}), 500
...
def register():
    ...
    try:
        ...
        if response.status_code == 200:  # dal da-manager abbiamo in riscontro positivo
            ...
            return jsonify(
                {"error": "Internal server error", "encrypted_password": hashed_password, "password": password,
                 "salt": salt}), 500
    except requests.exceptions.RequestException as e:
        return jsonify({"error": "Could not connect to db_manager", "details": str(e)}), 500
...
def register_admin():
    ...
    try:
        ...
        if response.status_code == 200:  # dal da-manager abbiamo in riscontro positivo
            ...
            token = u.generate_token(email, "admin")
            u.set_auth_token(token)
            return jsonify({"ok": "Registrazione avvenuta con successo!", "token": token, "Salt": salt,
                            "hashed_password": hashed_password, "password": password}), 200
        elif response.status_code == 400:
            return jsonify({"error": "Invalid credentials"}), 400
        else:
            return jsonify({"error": "Internal server error", "Salt": salt, "hashed_password": hashed_password,
...
def update_user():
    ...
    #     return jsonify({"error": "Unauthorized. Role required","role":role,"token_data":token_data}), 403
    ...
    # inserisco i campi appena presi nelvettore di sanificazione
    # fields_to_process = [firstname, lastname, password, email, currencyAmount]
    # processed_fields = process_fields(fields_to_process)
    #
    # # prelevo i campi sanificati
    # firstname = processed_fields[0]
